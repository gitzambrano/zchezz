...
int board_is_attacked(const Board *b, int sq, int by) {
    ...
    if (by == COL_W) {
        /* Cheap tests first: pawn, knight, king — before magic lookups */
        ...
        uint64_t bq = b->bb[2] | b->bb[4];    /* WB|WQ */
        uint64_t rq = b->bb[3] | b->bb[4];    /* WR|WQ */
        ...
        uint64_t bq = b->bb[8] | b->bb[10];   /* BB|BQ */
        uint64_t rq = b->bb[9] | b->bb[10];   /* BR|BQ */
        if (bq && (bish_attacks(sq,occ) & bq)) return 1;
        if (rq && (rook_attacks(sq,occ) & rq)) return 1;
    }
    return 0;
