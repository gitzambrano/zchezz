/* main.c — Zchezz v5.07 UCI engine
 *
...
/* Copy a UCI go argument list while removing one standalone token.
 * Used to turn "go ponder ..." into the equivalent timed "go ..." on ponderhit. */
static void copy_without_token(const char *src, const char *remove,
                               char *dst, size_t dstsz) {
    const char *p = src;
    char tok[256];
    dst[0] = '\0';
    while (next_token(&p, tok, sizeof(tok))) {
        if (strcasecmp(tok, remove) == 0) continue;
        size_t used = strlen(dst);
        size_t add = strlen(tok) + (used ? 1u : 0u);
        if (used + add + 1u > dstsz) break;
        if (used) strcat(dst, " ");
        strcat(dst, tok);
    }
}
...
static int       g_ponder_active = 0;
/* 0 = hold ponder result, 1 = ponderhit (suppress), 2 = stop (emit). */
static volatile int g_ponder_release = 2;
static char      g_ponder_resume_line[2048] = "";

static void ponder_wait_release(void) {
    while (g_ponder_release == 0) {
        struct timespec ts = {0, 1000000};
        nanosleep(&ts, NULL);
    }
}

static void stop_running_search(int emit_ponder_bestmove) {
    if (g_searching) {
        if (g_ponder_active)
            g_ponder_release = emit_ponder_bestmove ? 2 : 1;
        g_stop_flag = 1;
...
static void *search_thread_fn(void *arg) {
    ...
    if (sta->own_book && sta->book_loaded) {
        ...
        if (book_probe(&sta->board, &book_move)) {
            ...
            if (sta->ponder_mode) ponder_wait_release();
            if (!(sta->suppress_bestmove ||
                  (sta->ponder_mode && g_ponder_release == 1))) {
                pthread_mutex_lock(&g_io_mutex);
...
static void cmd_go(const char *line) {
    ...
    g_ponder_active = ponder;
    if (ponder) copy_without_token(line, "ponder", g_ponder_resume_line, sizeof(g_ponder_resume_line));
    else g_ponder_resume_line[0] = '\0';
    ...
    g_sta.suppress_bestmove = 0;
    g_sta.ponder_mode = ponder;
    g_ponder_release = ponder ? 0 : 2;
...
int main(int argc, char **argv) {
    ...
    while (fgets(line, sizeof(line), stdin)) {
        ...
        if      (eat(&p, "uci"))        { cmd_uci(); }
            ...
            if (g_searching && g_ponder_active) {
                char resume[sizeof(g_ponder_resume_line)];
                strncpy(resume, g_ponder_resume_line, sizeof(resume) - 1);
                resume[sizeof(resume) - 1] = '\0';

                /* Release a possibly completed ponder search without
                 * emitting its bestmove, preserve TT/history, then resume
                 * with the normal clock budget. */
                g_ponder_release = 1;
                g_stop_flag = 1;
                pthread_join(g_search_thread, NULL);
                g_searching = 0;
                g_ponder_active = 0;
                if (resume[0]) cmd_go(resume);
            }
        }
        else if (eat(&p, "quit"))       {
            stop_running_search(1);
    ...
    /* EOF reached — terminate any running/held ponder search cleanly. */
    stop_running_search(1);
    book_close();
