...
/* Fast validation for moves recovered from the compact TT.
 * This checks move geometry and special-move consistency without generating
 * the full move list. King-safety legality remains the caller's responsibility. */
int board_move_is_pseudo_legal(const Board *b, const Move *m) {
    if (!b || !m) return 0;
    int from = m->from, to = m->to;
    if ((unsigned)from >= 64u || (unsigned)to >= 64u || from == to) return 0;

    uint8_t pc = b->b[from];
    uint8_t dst = b->b[to];
    if (!pc || PC_COLOR(pc) != b->turn) return 0;
    if (dst && PC_COLOR(dst) == b->turn) return 0;
    if (m->prom && (m->prom < 2 || m->prom > 5)) return 0;
    if (m->epc > 1 || m->castle > 4) return 0;
    ...
    if (m->castle) {
        if (type != 6 || m->prom || m->epc || dst) return 0;
        switch (m->castle) {
            case 1:
                return col == COL_W && from == 60 && to == 62 &&
                       (b->ca & CA_WK) && b->b[63] == WR && !b->b[61] && !b->b[62] &&
                       !board_is_attacked(b,60,COL_B) && !board_is_attacked(b,61,COL_B) &&
                       !board_is_attacked(b,62,COL_B);
            case 2:
                return col == COL_W && from == 60 && to == 58 &&
                       (b->ca & CA_WQ) && b->b[56] == WR && !b->b[59] && !b->b[58] && !b->b[57] &&
                       !board_is_attacked(b,60,COL_B) && !board_is_attacked(b,59,COL_B) &&
                       !board_is_attacked(b,58,COL_B);
            case 3:
                return col == COL_B && from == 4 && to == 6 &&
                       (b->ca & CA_BK) && b->b[7] == BR && !b->b[5] && !b->b[6] &&
                       !board_is_attacked(b,4,COL_W) && !board_is_attacked(b,5,COL_W) &&
                       !board_is_attacked(b,6,COL_W);
            case 4:
                return col == COL_B && from == 4 && to == 2 &&
                       (b->ca & CA_BQ) && b->b[0] == BR && !b->b[3] && !b->b[2] && !b->b[1] &&
                       !board_is_attacked(b,4,COL_W) && !board_is_attacked(b,3,COL_W) &&
                       !board_is_attacked(b,2,COL_W);
            default:
                return 0;
        }
    }

    if (type != 1 && (m->prom || m->epc)) return 0;

    if (type == 1) {
        int r = from >> 3, f = from & 7;
        int dir = col == COL_W ? -8 : 8;
        int start_r = col == COL_W ? 6 : 1;
        int promo_r = col == COL_W ? 1 : 6;
        int ep_r = col == COL_W ? 3 : 4;
        int d = to - from;
        int df = (to & 7) - f;
        int promotion = (r == promo_r);

        if (m->epc) {
            if (m->prom || r != ep_r || b->ep != to || dst) return 0;
            if (!((df == -1 && d == dir - 1) || (df == 1 && d == dir + 1))) return 0;
            int cap_sq = col == COL_W ? to + 8 : to - 8;
            uint8_t expected = col == COL_W ? BP : WP;
            return (unsigned)cap_sq < 64u && b->b[cap_sq] == expected;
        }

        if (d == dir && df == 0 && !dst)
            return promotion ? (m->prom >= 2 && m->prom <= 5) : !m->prom;
        if (d == 2 * dir && df == 0 && r == start_r && !m->prom && !dst && !b->b[from + dir])
            return 1;
        if ((df == -1 && d == dir - 1) || (df == 1 && d == dir + 1)) {
            if (!dst || PC_COLOR(dst) == col) return 0;
            return promotion ? (m->prom >= 2 && m->prom <= 5) : !m->prom;
        }
        return 0;
    }

    if (m->prom || m->epc) return 0;
    switch (type) {
        case 2: return !!(NATK[from] & to_bb);
        case 3: return !!(bish_attacks(from, b->occ) & to_bb);
        case 4: return !!(rook_attacks(from, b->occ) & to_bb);
        case 5: return !!(queen_attacks(from, b->occ) & to_bb);
        case 6: return !!(KATK[from] & to_bb);
        default: return 0;
    }
}
