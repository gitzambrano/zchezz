...
/* ── Stockfish-like compact transposition table ────────────────
 * Three 10-byte entries fit in one 32-byte cluster (2 bytes padding).
 * Entry: key16, depth8, generation+bound, move16, score16, eval16.
 *
 * Unlike Stockfish, stale generations remain move-only hits here so this
 * experiment isolates layout/replacement from cross-game TT reuse.
 */
typedef struct {
    uint16_t key16;
...
static TTCluster32 *g_tt = NULL;
static size_t g_tt_clusters = 0;
static size_t g_tt_bytes = 0;
uint16_t TT_GEN = 0;

static inline uint8_t tt_gen5(void) { return (uint8_t)(TT_GEN & 31); }
static inline uint8_t tt_entry_gen(const TTEntry10 *e) { return e->genBound8 & 31; }
static inline int tt_entry_bound(const TTEntry10 *e) { return (e->genBound8 >> 5) & 3; }
static inline int tt_age(const TTEntry10 *e, uint8_t gen) { return (gen - tt_entry_gen(e)) & 31; }
static inline uint16_t tt_key16(uint64_t h) { return (uint16_t)h; }
static inline size_t tt_index(uint64_t h) {
#if defined(__SIZEOF_INT128__)
    return (size_t)(((__uint128_t)h * (__uint128_t)g_tt_clusters) >> 64);
#else
    return (size_t)(h % g_tt_clusters);
#endif
}
...
int tt_resize_mb(int mb) {
    if (mb < 1) mb = 1;
    if (mb > 1024) mb = 1024;
    size_t want = (size_t)mb * 1024u * 1024u;
    size_t clusters = want / sizeof(TTCluster32);
    ...
    g_tt_bytes = bytes;
    memset(g_tt, 0, g_tt_bytes);
    fprintf(stderr, "[TT] sf32: requested=%d MB actual=%.1f MB clusters=%zu entries=%zu entry=10B cluster=32B\n",
            mb, (double)bytes/(1024.0*1024.0), clusters, clusters*3);
    return 1;
}

int tt_hashfull_permille(void) {
    if (!g_tt || !g_tt_clusters) return 0;
    size_t total = g_tt_clusters * 3;
    size_t sample = total < 1000 ? total : 1000;
    size_t used = 0;
    TTEntry10 *flat = (TTEntry10 *)g_tt;
    uint8_t gen = tt_gen5();
    for (size_t i=0; i<sample; ++i)
        if (flat[i].depth8 && tt_age(&flat[i], gen) == 0) ++used;
    return sample ? (int)(used * 1000 / sample) : 0;
}

static inline void tt_prefetch(uint64_t hash) {
    if (g_tt && g_tt_clusters) __builtin_prefetch(&g_tt[tt_index(hash)], 0, 1);
}
...
/* ── Static-eval pawn correction history ──────────────────────
 * Learn systematic NNUE residuals from completed alpha-beta nodes. */
#define CORR_SIZE 8192
...
static inline uint32_t pawn_corr_key(const Board *b) {
    uint64_t x = b->bb[0] * 0x9E3779B97F4A7C15ULL;
    x ^= b->bb[6] * 0xC2B2AE3D27D4EB4FULL;
    x ^= x >> 33;
    x *= 0xFF51AFD7ED558CCDULL;
    x ^= x >> 29;
    return (uint32_t)x & CORR_MASK;
}

static inline void corr_update_entry(int16_t *entry, int target, int depth) {
    if (target > CORR_MAX) target = CORR_MAX;
    if (target < -CORR_MAX) target = -CORR_MAX;
    int weight = depth * 2;
    if (weight < 2) weight = 2;
    if (weight > 16) weight = 16;
    int cur = *entry;
    int next = (cur * (16 - weight) + target * weight) / 16;
    if (next > CORR_MAX) next = CORR_MAX;
    if (next < -CORR_MAX) next = -CORR_MAX;
    *entry = (int16_t)next;
}
...
/* Compact 16-bit move: 6 from + 6 to + 4 special.
 * special 0=normal, 1..4=promotion N/B/R/Q, 5=ep, 6..9=castle 1..4. */
static inline uint16_t tt_pack_move16(const Move *m) {
    if (!m || !(m->from || m->to)) return 0;
    unsigned sp = 0;
    if (m->prom >= 2 && m->prom <= 5) sp = (unsigned)m->prom - 1;
    else if (m->epc) sp = 5;
    else if (m->castle >= 1 && m->castle <= 4) sp = 5u + (unsigned)m->castle;
    return (uint16_t)((m->from & 63) | ((m->to & 63) << 6) | ((sp & 15) << 12));
}

static inline void tt_unpack_move16(uint16_t v, Move *m) {
    memset(m, 0, sizeof(*m));
    if (!v) return;
    m->from = v & 63;
    m->to = (v >> 6) & 63;
    unsigned sp = (v >> 12) & 15;
    if (sp >= 1 && sp <= 4) m->prom = (uint8_t)(sp + 1);
    else if (sp == 5) m->epc = 1;
    else if (sp >= 6 && sp <= 9) m->castle = (uint8_t)(sp - 5);
}

/* Store/probe in a three-entry 32-byte cluster. */
static void tt_store(uint64_t hash, int score,
                     int depth, int flag, const Move *move,
                     int ply, int static_eval) {
    if (!g_tt || !g_tt_clusters) return;
    TTCluster32 *cl = &g_tt[tt_index(hash)];
    uint16_t key = tt_key16(hash);
    uint8_t gen = tt_gen5();
    int sd = depth < 0 ? 0 : (depth > 253 ? 253 : depth);
    uint8_t enc_depth = (uint8_t)(sd + 1);
    int stored_score = tt_score_store(score, ply);
    if (stored_score > 32767) stored_score = 32767;
    if (stored_score < -32768) stored_score = -32768;
    int se = static_eval;
    if (se > 32767) se = 32767;
    if (se < -32768) se = -32768;
    uint16_t pm = tt_pack_move16(move);

    TTEntry10 *target = NULL;
    for (int i=0; i<3; ++i) {
        TTEntry10 *e = &cl->e[i];
        if (e->depth8 && e->key16 == key) { target = e; break; }
        if (!e->depth8) { target = e; break; }
    }
    if (!target) {
        target = &cl->e[0];
        int worst = (int)target->depth8 - 8 * tt_age(target, gen);
        for (int i=1; i<3; ++i) {
            int value = (int)cl->e[i].depth8 - 8 * tt_age(&cl->e[i], gen);
            if (value < worst) { worst = value; target = &cl->e[i]; }
        }
    }

    int same = target->depth8 && target->key16 == key;
    int old_depth = target->depth8 ? (int)target->depth8 - 1 : -1;
    int age = target->depth8 ? tt_age(target, gen) : 31;

    if (same && !pm) pm = target->move16; /* preserve existing TT move */

    /* Stockfish-style bias: exact, different-key, older, or not-much-shallower wins. */
    if (!same || flag == TT_EXACT || age || sd + 6 > old_depth) {
        target->key16 = key;
        target->depth8 = enc_depth;
        target->genBound8 = (uint8_t)((gen & 31) | ((flag & 3) << 5));
        target->move16 = pm;
        target->score16 = (int16_t)stored_score;
        target->eval16 = (int16_t)se;
    } else if (pm && !target->move16) {
        target->move16 = pm;
    }
}
...
static int tt_probe(uint64_t hash, int ply, TTE *out) {
    ...
    for (int i=0; i<3; ++i) {
        TTEntry10 *e = &cl->e[i];
        if (!e->depth8 || e->key16 != key) continue;
        tt_unpack_move16(e->move16, &out->move);
        if (tt_age(e, gen) != 0) {
        ...
        out->score = tt_score_read((int)e->score16, ply);
        out->depth = (int)e->depth8 - 1;
        out->flag = tt_entry_bound(e);
        out->static_eval = (int)e->eval16;
        return 1;
    }
    return 0;
}
...
/* Exact pre-move check test for pruning gates.
 * Models post-move occupancy without make/unmake. Handles direct checks,
 * discovered slider checks, promotions, normal captures and en-passant. */
...
static int alpha_beta(SearchState *ss, Board *b, int depth, int alpha, int beta,
    ...
    *pv_len = 0;
    ...
    /* v3.26: check evasions remain fully searched without a blanket depth
     * extension; qsearch handles in-check horizon nodes. */
    ...
    if (!in_check) {
        ...
        int side_idx = b->turn == COL_W ? 0 : 1;
        int corr = ss->pawn_corr[side_idx][pawn_corr_key(b)];
        static_eval = raw_eval + corr;
    ...
    if (pv_ptr) {
        ...
        int mfr = m->from, mto = m->to;
        /* Compact TT tags can collide. Never trust a recovered TT move
         * until its geometry and special fields match the current board. */
        if (board_move_is_pseudo_legal(b, m)) {
    ...
    /* ── STAGE 2: Generate captures + promotions, score, pick-best ── */
    {
        ...
        /* ── STAGE 3: Quiet moves ────────────────────────────────── */
        {
            ...
            for (int i = 0; i < n_quiets; i++) {
                ...
                if (!in_check && !is_pv && is_quiet && legal_count > 0 && depth <= 4 &&
                    !is_killer &&
                    !(cur_prev_ft >= 0 && ss->counter_move[cur_prev_ft] == (mfr*64+mto)) &&
                    !move_gives_check_pre(b, m)) {
...
cutoff:
    ...
    /* Learn only from non-tactical best moves and safe alpha-beta bounds. */
    if (!in_check && raw_eval != TT_EVAL_NONE && best > -9000 && best < 9000 &&
        (best_move.from || best_move.to) && !ss->time_up) {
        int mto = best_move.to;
        int is_capture = !!(b->b[mto] || best_move.epc);
        int is_quiet = !is_capture && !best_move.prom && !best_move.castle;
        int bound_ok = (flag == TT_EXACT) ||
                       (flag == TT_LOWER && best > raw_eval) ||
                       (flag == TT_UPPER && best < raw_eval);
        if (is_quiet && bound_ok) {
            int side_idx = b->turn == COL_W ? 0 : 1;
            int target = best - raw_eval;
            corr_update_entry(&ss->pawn_corr[side_idx][pawn_corr_key(b)], target, depth);
        }
    }
...
void search_init(void) {
    ...
    /* Default UCI Hash is 64 MB; setoption Hash can resize later. */
    if (!g_tt) {
        if (!tt_resize_mb(64)) { fprintf(stderr, "[TT] allocation failed\n"); abort(); }
    } else tt_clear();
}
