...
def test_key_docs_name_supported_profiles():
    for rel in ("AGENTS.md", "CLAUDE.md", "Readme.md", "docs/architecture.md", "docs/nnue.md", "docs/repository-layout.md"):
        ...
        assert "v331" in text, rel
        assert "v507" in text, rel
...
def test_v507_nnue_source_comments_match_compact_architecture():
    text = (ROOT / "engine/c/zchezz_v507/nnue.c").read_text(encoding="utf-8")
...
def test_native_launchers_default_to_v331():
    ...
    assert "set ENGINE=v331" in bat
    assert 'VERSION="${1:-v331}"' in termux
