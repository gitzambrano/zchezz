...
def test_key_docs_name_supported_profiles():
    for rel in ("AGENTS.md", "CLAUDE.md", "Readme.md", "docs/architecture.md", "docs/nnue.md", "docs/repository-layout.md"):
        ...
        assert "v328" in text, rel
        assert "v506" in text, rel
...
def test_v506_nnue_source_comments_match_compact_architecture():
    text = (ROOT / "engine/c/zchezz_v506/nnue.c").read_text(encoding="utf-8")
...
def test_native_launchers_default_to_v328():
    ...
    assert "set ENGINE=v328" in bat
    assert 'VERSION="${1:-v328}"' in termux
