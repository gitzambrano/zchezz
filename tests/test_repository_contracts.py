...
def test_supported_profiles_and_default():
    assert DEFAULT_PROFILE=="v330"; assert set(PROFILES)=={"v325","v326","v327","v328","v329","v330","v500","v505","v506"}; assert (ROOT/"engine/ACTIVE_ENGINE").read_text(encoding="utf-8").strip()=="v330"
...
def test_makefile_defaults_are_safe():
    text=(ROOT/"engine/build/Makefile").read_text(encoding="utf-8"); assert re.search(r"^ENGINE\s*\?=\s*v330\b",text,re.M); assert re.search(r"^TOOLS_ENGINE\s*\?=\s*v506\b",text,re.M); assert "-DNO_TABLEBASES" in text
