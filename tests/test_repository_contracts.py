...
def test_makefile_defaults_are_safe():
    text=(ROOT/"engine/build/Makefile").read_text(encoding="utf-8"); assert re.search(r"^ENGINE\s*\?=\s*v331\b",text,re.M); assert re.search(r"^TOOLS_ENGINE\s*\?=\s*v507\b",text,re.M); assert "-DNO_TABLEBASES" in text
