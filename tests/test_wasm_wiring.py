...
def test_wasm_source_is_shared():
    expected=build_root()/"zchezz_wasm.html"; assert wasm_source("v327")==expected; assert wasm_source("v506")==expected
...
def test_browser_e2e_targets_generated_bundle():
    m=_runner(); command=next(s.command for s in m.profiles("v506","v327")["web"] if s.name=="browser e2e"); assert "--html" in command and "zchezz_bundle.html" in command
