...
def main() -> int:
    ...
    if set(PROFILES)!={"v325","v326","v327","v328","v329","v500","v505","v506"}: error(f"supported profiles drifted: {sorted(PROFILES)}")
    ...
    if not re.search(r'^ENGINE\s*\?=\s*v329\b',make,re.M): error('Makefile default must be v329')
