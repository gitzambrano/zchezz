#!/usr/bin/env python3
"""Train only a 31->48 global-information projection on top of frozen NNU4.

The installed NNU4 base is never modified.  A shared zero-initialized projection
is added to each perspective's L1 pre-activation before SCReLU.  Training uses
root-probe child ranking/policy plus a replay trust penalty; therefore any playing
strength change can be attributed to the additional global information channel.
"""
...
from train.model import (  # noqa: E402
    NNUE, QA, QA_EFF, QB, fake_quant_bias_int32, fake_quant_int16,
    fake_quant_int8,
)
...
FEATURE_SETS = {
    "all": list(range(31)),
    "aggregate": list(range(14)),
    "relational": list(range(14, 31)),
}


class GlobalProjectionNNU4(nn.Module):
    def __init__(self, base: NNUE, feature_set: str):
        ...
        self.base.eval()
        self.global_proj = nn.Parameter(torch.zeros(GLOBAL_DIM, 48))
        mask = torch.zeros(GLOBAL_DIM, 1)
        mask[FEATURE_SETS[feature_set], 0] = 1.0
        self.register_buffer("feature_mask", mask)

    def forward(self, stm_idx, stm_off, opp_idx, opp_off, g_stm, g_opp):
        b = self.base
        w1 = fake_quant_int16(b.l1.weight, QA)
        b1 = fake_quant_bias_int32(b.l1_bias, float(QA))
        # Runtime stores global features as Q8 with 1.0 -> 256.
        gs = torch.round(g_stm * 256.0) / 256.0
        go = torch.round(g_opp * 256.0) / 256.0
        gw = fake_quant_int16(self.global_proj * self.feature_mask, QA)

        a_stm = F.embedding_bag(stm_idx, w1, stm_off, mode="sum") + b1 + gs @ gw
        a_opp = F.embedding_bag(opp_idx, w1, opp_off, mode="sum") + b1 + go @ gw
        h_stm = b.act1(a_stm)
        h_opp = b.act1(a_opp)
...
def _encoded(fens: list[str], device: torch.device):
    ...
    gs, go = encode_global_features(fens)
    return (
        ...
        torch.as_tensor(go, dtype=torch.float32, device=device),
    )


def forward_global(model: GlobalProjectionNNU4, fens: list[str], device: torch.device):
...
def policy_rank_loss(model, groups: list[Group], device, temperature_cp: float,
                     rank_weight: float, min_margin_cp: float):
    fens: list[str] = []
    boundaries = [0]
    rank_scores = []
    target_policy = []
    ...
    p = forward_global(model, fens, device).clamp(1e-6, 1 - 1e-6)
...
def trust_penalty(model, base, fens: list[str], device, scale_cp: float):
    pg = forward_global(model, fens, device).clamp(1e-6, 1 - 1e-6)
    with torch.no_grad():
...
def evaluate(model, base, groups, replay, device, args):
    ...
    obj = float(loss.cpu()) + args.preserve_weight * float(pen.cpu())
    return {
        "objective": obj, "policy": float(pol.cpu()), "rank": float(rank.cpu()),
...
def main() -> int:
    ...
    ap.add_argument("--seed", type=int, default=5073101)
    ...
    model = GlobalProjectionNNU4(base, a.feature_set).to(device)

    # Zero projection must be exactly the base float/QAT function.
    probe = vr[:min(64, len(vr))]
    with torch.no_grad():
        p0 = forward_global(model, probe, device)
    ...
    if err > 1e-7:
        raise SystemExit(f"zero projection is not base-identical: {err}")

    opt = torch.optim.AdamW([model.global_proj], lr=a.lr, weight_decay=a.weight_decay)
    best = evaluate(model, base, vg, vr, device, a)
    best_state = copy.deepcopy(model.global_proj.detach().cpu())
    best_epoch = 0
    ...
    for epoch in range(1, a.epochs + 1):
        ...
        for start in range(0, len(tg), max(1, a.batch_groups)):
            ...
            # Explicit small L2 term keeps the new side-channel conservative.
            reg = torch.mean((model.global_proj * model.feature_mask) ** 2)
            loss = teacher + a.preserve_weight * pen + 1e-4 * reg
            loss.backward()
            # Permanently mask excluded feature rows.
            if model.global_proj.grad is not None:
                model.global_proj.grad.mul_(model.feature_mask)
            torch.nn.utils.clip_grad_norm_([model.global_proj], 5.0)
            opt.step()
            with torch.no_grad():
                model.global_proj.mul_(model.feature_mask)
                model.global_proj.clamp_(-32767.0/QA, 32767.0/QA)
    ...
    with torch.no_grad():
        model.global_proj.copy_(best_state.to(device))
    ...
    torch.save({
        ...
        "global_proj": model.global_proj.detach().cpu(),
